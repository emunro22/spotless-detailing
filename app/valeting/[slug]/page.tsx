...
export async function generateStaticParams() {
  return getAllValetingSlugs().map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const page = findValetingPage(slug);
  if (!page) return {};
...
export default async function ValetingSlugPage({ params }: Props) {
  const { slug } = await params;
  const page = findValetingPage(slug);
  if (!page) notFound();
      ...
      <section className="relative py-16 md:py-24">
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {SERVICES.filter((s) => s.slug !== 'maintenance-plan').map(
              (service) => (
                <Link
                  key={service.slug}
                  href="/services"
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <div className="text-sm text-cream font-medium">
                    Glasgow & 15 miles from Uddingston
                  </div>
                </div>
              </div>
            </div>
